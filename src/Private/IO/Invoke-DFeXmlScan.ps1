...
already known are skipped immediately without opening the XML content,
making subsequent executions significantly faster on large directories.

Malformed XML and XML content that cannot be classified are treated as
content rejections. They increment FilesIgnored and do not interrupt the
scan.

Operational failures are not converted to ignored files. Failures while
reading indexed hashes, enumerating the directory, hashing a source file,
or persisting metadata terminate the scan so the caller can surface the
failure and prevent an incomplete delivery.
...
function Invoke-DFeXmlScan {
    ...
    # Any failure here is operational and must abort the scan. Proceeding with
    # an empty set after a database read failure could cause already indexed
    # files to be processed again and would hide index health problems.
    $knownHashes = [System.Collections.Generic.HashSet[string]]::new(
        [System.StringComparer]::OrdinalIgnoreCase
    )

    Get-DFeIndexedHash -Cnpj $Cnpj |
        ForEach-Object { $null = $knownHashes.Add($_) }

    Write-Verbose -Message "[$Cnpj] $($knownHashes.Count) indexed hash(es) loaded."

    # Enumerating the source tree is part of the scan contract. Partial
    # enumeration cannot be treated as a successful scan because the result
    # would no longer describe the requested XmlPath completely.
    $getChildParams = @{
        LiteralPath = $XmlPath
        Filter      = '*.xml'
        Recurse     = $true
    ...
    foreach ($file in $files) {
        ...
        try {
            ...
            if ($_.FullyQualifiedErrorId -like 'InvalidXml*') {
                $ignored++

                Write-Verbose -Message (
                    "[$Cnpj] Skipped (invalid XML): '$($file.Name)' - " +
                    "$($_.Exception.Message)."
                )
